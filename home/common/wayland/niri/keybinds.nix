...
let
  ...
  baseBinds = with config.lib.niri.actions; {
    "xf86audioraisevolume".action = spawn wpctl [
      "set-sink-volume"
      "@DEFAULT_SINK@"
      "0.05+"
    ];
    "xf86audiolowervolume".action = spawn wpctl [
      "set-sink-volume"
      "@DEFAULT_SINK@"
      "0.05-"
    ];
  ...
  # :Quickshell
  qsBinds = with config.lib.niri.actions; {
  ...
  # :Eww
  ewwBinds = with config.lib.niri.actions; {
...
in
{
  programs.niri.settings.binds = baseBinds // (if host == "Rain" then ewwBinds else qsBinds);
}
