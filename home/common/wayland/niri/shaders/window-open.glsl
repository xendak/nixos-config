...
vec4 open_color(vec3 coords_geo, vec3 size_geo) {
    ...
    float hideThreshold = mix(1.0 + SMOKE_WIDTH, -SCORCH_WIDTH, progress);
    
    vec2 scorchRange = vec2(hideThreshold - SCORCH_WIDTH, hideThreshold);
    ...
    float diag = (uv.x + (1.0 - uv.y)) / 1.414;
    vec2 noiseUV = uv / uScale * size_geo.xy / 1.5;
    float smokeNoise = fbm(noiseUV * 0.01 + uSeed + progress * vec2(0.3, 0.3));
    float gradient = mix(diag, smokeNoise, 100.0 * uTurbulence * uScale / max(size_geo.x, size_geo.y));
    ...
    if (min(burnRange.x, flameRange.x) < gradient && gradient < max(burnRange.y, flameRange.y)) {
        float flameNoise = fbm(noiseUV * 0.02 + uSeed + progress * vec2(1.0, 1.0));
