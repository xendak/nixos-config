...
# Assuming util.fish does not contain hyprctl commands
...
if set -q _flag_h
    ...
    if set -q _flag_f
        ...
        if set -q _flag_F
            ...
            # MODIFIED LINE: Changed from hyprctl to niri msg and updated jq filter
            set -l screen_size (niri msg -j get-outputs | jq -r 'max_by(.mode.width * .mode.height) | "\(.mode.width)\n\(.mode.height)"')
