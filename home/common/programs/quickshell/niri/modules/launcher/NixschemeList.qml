...
PathView {
    ...
    model: ScriptModel {
        ...
        values: {
            const list = Nixschemes.fuzzyQuery(search);
            if (list.length > 1 && list.length % 2 === 0)
                list.length -= 1;
            return list;
        }
        onValuesChanged: root.currentIndex = search ? 0 : values.findIndex(w => w.path === Nixschemes.actualCurrent)
    }

    Component.onCompleted: currentIndex = Nixschemes.list.findIndex(w => w.path === Nixschemes.actualCurrent)
    Component.onDestruction: Nixschemes.stopPreview()

    onCurrentItemChanged: {
        if (currentItem)
            Nixschemes.preview(currentItem.modelData.path);
    }
