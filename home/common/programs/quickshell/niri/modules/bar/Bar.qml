...
Item {
    ...
    function checkPopout(y: real): void {
        ...
        const ry = recorder.y;
        const rh = recorder.implicitHeight;

        if (y > ty && y < ty + th) {
            const index = Math.floor(((y - ty) / th) * trayItems.length);
            const item = trayItems[index];
            if (!item)
                return;

            popouts.currentName = `traymenu_${item.modelData.id}`;
            popouts.currentCenter = Qt.binding(() => tray.y + item.y + item.implicitHeight / 2);
            popouts.hasCurrent = true;
        } else if (y >= ry && y <= ry + rh + spacing) {
            popouts.currentName = "recorder";
            popouts.currentCenter = Qt.binding(() => recorder.y + recorder.implicitHeight / 2);
            popouts.hasCurrent = true;
    ...
    Item {
        ...
        StyledRect {
            ...
            MouseArea {
                ...
                onWheel: event => {
                    if (event.angleDelta.y < 0 || Niri.activeWsId > 0) {
                        const direction = event.angleDelta.y > 0 ? "up" : "down";
                        Niri.dispatch(`focus-workspace-${direction}`);
                    }
                }
            }
