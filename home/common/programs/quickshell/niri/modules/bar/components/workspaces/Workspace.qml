...
Item {
    ...
    // onIsOccupiedChanged: {
    //     console.log(`Workspace.qml - Button ${root.ws} changed occupied status to: ${isOccupied}`);
    // }
    ...
    StyledText {
        ...
        animate: true
        text: Niri.activeWsId === root.ws ? activeLabel : root.isOccupied ? occupiedLabel : label
        color: BarConfig.workspaces.occupiedBg || root.isOccupied || Niri.activeWsId === root.ws ? Colours.palette.m3onSurface : Colours.palette.m3outlineVariant
    ...
    Loader {
        ...
        sourceComponent: Column {
            ...
            Repeater {
                model: ScriptModel {
                    values: Niri.clients ? Niri.clients.filter(c => c.workspace && c.workspace.id === root.ws) : []
                }
