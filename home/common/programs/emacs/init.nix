...
{
  home.file = {
    ".config/emacs/init.el".text = # clisp
      ''
        (require 'package)
        (require 'fzf)
        ...
        ;; Function to reload keybinds on the fly, while i learn this..
        (defun my/reload-keybindings ()
          (interactive)
          (let ((bindings-file (expand-file-name "binds.el" user-emacs-directory)))
            (if (file-exists-p bindings-file)
                (progn
                  (load-file bindings-file)
                  (message "Successfully reloaded keybindings from binds.el"))
              (message "Keybindings file not found: %s" bindings-file))))
