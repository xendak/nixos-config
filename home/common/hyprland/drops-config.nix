{ config, colorscheme }: ''
  ...
  general {
    ...
    col.inactive_border=0xa0${colorscheme.palette.base02}
  ...
  decoration {
    ...
    col.shadow_inactive=0x66${colorscheme.palette.base00}
    col.shadow=0xf0${colorscheme.palette.base0C}
  ...
  # Startup
  exec-once=ags -c $HOME/Flake/home/drops/ags/config.js
...
#  bind=SUPER,r,exec,rofi -show drun -matching fuzzy -sorting-method fzf -sort -theme "$HOME/.config/rofi/config.rasi"
  ...
  bind=SUPER,X,exec,sh "$HOME/.config/rofi/powermenu.sh"
...
#  bind=SUPERSHIFT,b,exec,pkill -USR1 waybar || waybar # profile button
   bind=SUPERSHIFT,b,exec,ags -t quicksettings
   bind=SUPERCONTROL,b,exec,ags -t datemenu
   bind=SUPERSHIFT,tab,exec,ags -t overview
  ...
  bind=SUPER,bracketright,exec,brightnessctl set 5%+
  bind=SUPER,bracketleft,exec,brightnessctl set 5--%-
  bind=SUPERSHIFT,period,exec,wpctl set-volume @DEFAULT_SINK@ 0.05+
  bind=SUPERSHIFT,comma,exec,wpctl set-volume @DEFAULT_SINK@ 0.05-
  bind=SUPERSHIFT,slash,exec,wpctl set-mute @DEFAULT_SINK@ toggle
